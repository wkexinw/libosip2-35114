...
int osip_message_set_authorization(osip_message_t *sip, const char *hvalue)
{
  ...
  printf("[%s:%d]algorithm:%s\n", __FILE__, __LINE__, authorization->algorithm);
  if (i != 0)
  {
...
int osip_authorization_parse(osip_authorization_t *auth, const char *hvalue)
{
  ...
  for (;;)
  {
    ...
    printf("[%s:%d]random1: %s\n", __FILE__, __LINE__, auth->random1);
    if (i != 0)
      return i;
    ...
    printf("[%s:%d]random2: %s\n", __FILE__, __LINE__, auth->random2);
    if (i != 0)
      return i;
    ...
    printf("[%s:%d]serverid: %s\n", __FILE__, __LINE__, auth->serverid);
    if (i != 0)
      return i;
    ...
    printf("[%s:%d]deviceid: %s\n", __FILE__, __LINE__, auth->deviceid);
    if (i != 0)
      return i;
    ...
    /*************************WARNING****************************/
    i = __osip_quoted_string_set("sign2", space, &(auth->sign2), &next);
    printf("[%s:%d]sign2: %s\n", __FILE__, __LINE__, auth->sign2);
    if (i != 0)
      return i;
...
char *osip_authorization_get_sign2(osip_authorization_t *authorization)
{
  return authorization->sign2;
}

void osip_authorization_set_sign2(osip_authorization_t *authorization, char *sign2)
{
  authorization->sign2 = (char *)sign2;
}
...
int osip_authorization_to_str(const osip_authorization_t *auth, char **dest)
{
  ...
  *dest = tmp;
  ...
  if (auth->sign2 != NULL)
  {
    ...
    first = 0;
    tmp = osip_strn_append(tmp, " sign2=", 7);
    tmp = osip_str_append(tmp, auth->sign2);
  }
...
int osip_authorization_clone(const osip_authorization_t *auth, osip_authorization_t **dest)
{
  ...
  *dest = NULL;
  ...
  if (auth->sign2 != NULL)
  {
    au->sign2 = osip_strdup(auth->sign2);
